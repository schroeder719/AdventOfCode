...
class Intcode():
    ...
    def reset(self):
        self.code = self.reset_code + 5000 * [0,]
        self.relative_base = 0
        self.program_counter = 0
        self.input_que = queue.Queue()
        self.output_que = queue.Queue()
...
if curses_on:
    ...
    while True:
        ...
        if result == intcode.OUTPUT_AVAILABLE:
            ...
            #if intcode.input_que.empty():
            if not new_result:
                print("ERROR, no new result")
                new_result = False
            if last_result not in ['.','#']:
                print("return value unexpected: {}".format(last_result))
                break
            if mode == FIND_UL:
                if last_result == '.':
                    x_start+=1
                    if x_start > 10000:
                        x_start = last_first_x
                        y_start+=1 
                    inpt_x = x_start
                    inpt_y = y_start
                else:
                    last_first_x = x_start
                    mode = FIND_UR
                    inpt_x = x_start+100
                    inpt_y = y_start
            elif mode == FIND_UR:
                ...
                if last_result == '.':
                    ...
                    print("Found Result: {},{}  puzzle answer:{}".format(x_start,y_start,x_start*10000+y_start))
                    done = True
            print("M:{}, x:{},y:{}".format(mode,inpt_x,inpt_y))
            intcode.input(inpt_x)
            intcode.input(inpt_y)
        elif result == intcode.COMPLETE:
            if done:
                print("Complete")
                break
